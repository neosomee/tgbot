...
def info(message):
    if message.text == '🛍 Товары':
        ...
        bot.send_message(message.chat.id,
                         'Мед обладает широким спектром применения, его в 1-ую очередь используют при простуде,также он применяется как общеукрепяющее средство и как отличное средство для устойчивой работы сердца.\n '
        ...
        markup.row(button1)
        bot.send_message(message.chat.id,
                         'Мёд обладает целебным действием, усиливает обмен веществ, ускоряет восстановлению тканей, оказывает противовоспалительное, рассасывающее и бодрящее действие.\n '
                         'Объем и цены: 0.5=300р, 1л=600р, 1.5л=900р,5л=3000р. '
        ...
        markup.row(button1)
        bot.send_message(message.chat.id,
                         'Мед в сотах особенно полезен, поскольку в нем присутствуют ценнейшие для здоровья вещества - прополис, воск, перга. В обработанном продукте такие компоненты отсутствуют, что по определению делает его свойства менее полезными\n '
        ...
        markup.row(button1)
        bot.send_message(message.chat.id,
                         'Входящие в состав продукта элементы оказывают влияние на выносливость, способствуют ускорению роста мышечной массы, его применение активизирует выработку гемоглобина и скорость усвоения кальция.\n '
        ...
        bot.send_message(message.chat.id,
                         'Хочешь следить за новостями разработчика в его телеграмм канале?:\nСсылка: https://t.me/logneosome')
